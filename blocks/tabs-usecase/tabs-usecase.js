...
export default async function decorate(block) {
  ...
  tablist.className = 'tabs-usecase-list';
  ...
  tabHeadings.forEach((tab, i) => {
    ...
    tabpanel.className = 'tabs-usecase-panel';
    tabpanel.id = id;
    tabpanel.setAttribute('aria-hidden', !!i);
    tabpanel.setAttribute('aria-labelledby', `tab-${id}`);
    tabpanel.setAttribute('role', 'tabpanel');

    // decorate CTA links inside the panel: first link is a primary button,
    // subsequent links stay as plain green text links. Drop empty links.
    const links = [...tabpanel.querySelectorAll('a')].filter((link) => {
    ...
    button.className = 'tabs-usecase-tab';
    button.id = `tab-${id}`;
